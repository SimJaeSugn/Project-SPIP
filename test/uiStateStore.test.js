...
test('normalizeHomeLayout — 화이트리스트 외/중복 제거 + 누락 보충(끝)', () => {
  ...
  assert.deepStrictEqual(r, ['mail', 'attention', 'productivity', 'activity', 'todos', 'disk', 'aiusage', 'shelf', 'shelfWide', 'scratchpad', 'commitHeatmap', 'systemStatus', 'explorer', 'featureAdd']);
  // 항상 화이트리스트 전체의 순열
...
test('defaultState/normalizeState — hiddenWidgets 시드(신규 위젯 기본 숨김) / 현재버전 입력은 무시드', () => {
  // [SH-1 PM#3 / Phase 3·G / 탐색기] 신규 설치 기본 숨김 시드 = DEFAULT_HIDDEN_WIDGETS.
  assert.deepStrictEqual(store.defaultState().hiddenWidgets, store.DEFAULT_HIDDEN_WIDGETS);
  assert.deepStrictEqual(store.DEFAULT_HIDDEN_WIDGETS, ['shelf', 'shelfWide', 'scratchpad', 'commitHeatmap', 'systemStatus', 'explorer']);
...
test('write/read — homeLayout 라운드트립 보존 (C-M-1)', () => {
  const file = tmpFile();
  const custom = ['mail', 'attention', 'productivity', 'activity', 'todos', 'disk', 'aiusage', 'shelf', 'shelfWide', 'scratchpad', 'commitHeatmap', 'systemStatus', 'explorer', 'featureAdd'];
