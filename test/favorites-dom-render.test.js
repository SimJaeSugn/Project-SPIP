...
test('실 렌더: 현재 슬라이드(is-current)에 카드가 가시(버튼만 있는 상태가 아님)', async () => {
  const { root } = mount(['a', 'b', 'c']);
  await flush();
  const current = root.querySelectorAll('.fav-carousel__slide.is-current .fav-card');
  assert.strictEqual(current.length, 1, '현재 슬라이드 카드 1장 노출');
...
test('빈 상태: 즐겨찾기 0 → 빈 안내 표시 + .fav-card 0', async () => {
  const { root } = mount([]);
  await flush();
...
test('좌우 이동: slideBy(+1) 시 현재 슬라이드 인덱스/aria-label 갱신', async () => {
  const { root, handle } = mount(['a', 'b', 'c']);
  await flush();
  assert.strictEqual(handle._state.index, 0);
  handle.slideBy(1);
  assert.strictEqual(handle._state.index, 1);
  const pos = root.querySelector('.fav-ctrl__pos');
  assert.strictEqual(pos.textContent, '2 / 3');
  handle.slideBy(-1);
  assert.strictEqual(handle._state.index, 0);
});

test('즐겨찾기 해제 후 인덱스 보정: 마지막 슬라이드에서 해제 시 index clamp', async () => {
  const { root, handle } = mount(['a', 'b', 'c']);
  await flush();
  handle.goTo(2); // 마지막 슬라이드(Gamma)
  assert.strictEqual(handle._state.index, 2);
  // Gamma(c) 해제 → 2장 남고 index 는 1 로 보정되어야 함.
  const unfavBtn = root.querySelectorAll('.fav-carousel__slide.is-current .fav-act--star')[0];
  assert.ok(unfavBtn, '현재 슬라이드 해제 버튼 존재');
  unfavBtn._dispatch('click', { stopPropagation() {} });
  await flush();
  const cards = root.querySelectorAll('.fav-card');
  assert.strictEqual(cards.length, 2, '해제 후 카드 2장');
  assert.ok(handle._state.index <= 1, '제거 후 인덱스 clamp(<=1)');
  const names = root.querySelectorAll('.fav-card__name').map((n) => n.textContent);
