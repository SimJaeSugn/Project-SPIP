...
test('setHomeLayout — 정규화·영속·응답(중복/미지/비배열 흡수)', () => {
  ...
  assert.strictEqual(r.ok, true);
  assert.deepStrictEqual(r.homeLayout, ['mail', 'attention', 'productivity', 'activity', 'todos', 'disk', 'aiusage', 'shelf', 'shelfWide', 'scratchpad', 'commitHeatmap', 'systemStatus', 'explorer', 'featureAdd']);
