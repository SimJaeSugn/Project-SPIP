...
test('R-30 — 설계 §8 매핑표대로 섹션 배치', () => {
  ...
  assert.deepStrictEqual(byId.integration, ['tools', 'mail', 'briefing']); // [M13] 브리핑 AI 추가
...
test('R-30 — 각 섹션은 정확히 하나의 카테고리에만 속한다(중복 배치 0)', () => {
  ...
  // 기존 + 신설(단축키·정보·[M13]브리핑) 섹션 모두 배치됐는지(누락 0).
  const all = ['roots', 'exclude', 'detect', 'scanOptions', 'tools', 'mail', 'briefing', 'theme', 'shortcuts', 'info', 'update'];
