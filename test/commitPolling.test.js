...
test('R-31/M10-P1 — 폴링 콜백: deferred 보류(플래그 설정) + silent refreshCommitActivity 경유', () => {
  ...
  // [M10-P1] deferred 시 _pendingCommitRefresh 플래그 설정, 비보류 시 silent 폴링.
  assert.ok(/_pendingCommitRefresh\s*=\s*true/.test(body), 'deferred 시 보류 플래그 설정(무재시도 보강)');
  assert.ok(/refreshCommitActivity\(\{\s*silent:\s*true\s*\}\)/.test(body), 'silent 폴링(진입 render 생략)');
});
