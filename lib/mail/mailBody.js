...
const { decodeMimeHeader, charsetToNode } = require('./imapProtocol');
...
/** 본문 디코드: CTE(base64/quoted-printable/7bit·8bit) + charset → 유니코드 문자열. */
function decodeBody(body, cte, charset) {
  body = String(body == null ? '' : body);
  const enc = String(cte || '').toLowerCase().trim();
  try {
    if (enc === 'base64') return Buffer.from(body.replace(/\s+/g, ''), 'base64').toString(charsetToNode(charset));
    if (enc === 'quoted-printable') return Buffer.from(qpDecode(body), 'latin1').toString(charsetToNode(charset));
  } catch (_) { /* 디코드 실패 → 원문 */ }
  // 7bit/8bit/none: 리더가 이미 UTF-8로 디코드한 문자열이라 그대로(UTF-8 본문 기준).
  return body;
}
