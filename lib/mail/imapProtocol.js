'use strict';
/**
 * lib/mail/imapProtocol.js — IMAP 응답 파싱·인용 (순수 함수, 헤드리스 단위테스트)
 *
 * 새 메일 감지에 필요한 최소 명령(LOGIN·STATUS·LOGOUT)의 응답만 다룬다. FETCH/literal은
 * 쓰지 않으므로 본 모듈도 단순 라인 파싱만 한다. 네트워크 I/O는 imapClient.js가 담당.
 *
 * 외부 의존성 0.
 */

/** 문자열을 IMAP quoted-string으로 인용한다(역슬래시·따옴표 이스케이프). LOGIN 인자 등에 사용. */
function imapQuote(s) {
  return '"' + String(s == null ? '' : s).replace(/\\/g, '\\\\').replace(/"/g, '\\"') + '"';
}

/**
 * 태그드 응답 라인 파싱: `A1 OK ...` → { tag, status, text }.
 *   비태그드(`*`)이거나 형식 불일치, (tag 지정 시) 태그 불일치면 null.
 */
function parseTaggedLine(line, tag) {
  if (typeof line !== 'string') return null;
  // 태그는 영숫자(A1 등)만 — 비태그드('*')·연속('+')은 의도적으로 제외.
  const m = line.match(/^([A-Za-z0-9]+)\s+(OK|NO|BAD)\b\s*(.*)$/i);
  if (!m) return null;
  if (tag && m[1] !== tag) return null;
  return { tag: m[1], status: m[2].toUpperCase(), text: m[3] || '' };
}

/**
 * STATUS 응답의 숫자 항목을 추출한다(키는 소문자화).
 *   `* STATUS "INBOX" (MESSAGES 3 UNSEEN 1 UIDNEXT 5)` → { messages:3, unseen:1, uidnext:5 }
 *   괄호/짝이 없으면 빈 객체.
 */
function parseStatusItems(line) {
  const out = {};
  if (typeof line !== 'string') return out;
  const open = line.indexOf('(');
  const close = line.lastIndexOf(')');
  if (open < 0 || close <= open) return out;
  const body = line.slice(open + 1, close).trim();
  if (!body) return out;
  const toks = body.split(/\s+/);
  for (let i = 0; i + 1 < toks.length; i += 2) {
    const key = toks[i].toLowerCase();
    const num = Number(toks[i + 1]);
    if (Number.isFinite(num)) out[key] = num;
  }
  return out;
}

/** UID SEARCH 응답 → UID 배열. `* SEARCH 1 2 3` → [1,2,3]. 미일치/빈 응답은 []. */
function parseSearchUids(line) {
  if (typeof line !== 'string') return [];
  const m = line.match(/^\*\s+SEARCH\b(.*)$/i);
  if (!m) return [];
  return m[1].trim().split(/\s+/).filter(Boolean).map((t) => Number(t)).filter((n) => Number.isInteger(n) && n > 0);
}

/**
 * IMAP S-expression 파서(순수). 따옴표문자열·NIL·원자·중첩 괄호를 JS 값/배열로.
 *   리터럴({n})은 리더 단계에서 이미 따옴표 문자열로 정규화됐다고 가정한다(imapClient._tryTakeLine).
 * @param {string} s '(' 로 시작하는(또는 단일 값) IMAP 식
 * @returns {*} 문자열 | null(NIL) | 배열(중첩)
 */
function parseSexp(s) {
  if (typeof s !== 'string') return null;
  let i = 0;
  const skipWs = () => { while (i < s.length && s[i] === ' ') i++; };
  function parseString() {
    i++; // 여는 따옴표
    let out = '';
    while (i < s.length) {
      const c = s[i++];
      if (c === '\\') { out += (i < s.length ? s[i++] : ''); }
      else if (c === '"') break;
      else out += c;
    }
    return out;
  }
  function parseList() {
    i++; // '('
    const arr = [];
    while (i < s.length) {
      skipWs();
      if (s[i] === ')') { i++; break; }
      arr.push(parseValue());
    }
    return arr;
  }
  function parseValue() {
    skipWs();
    const c = s[i];
    if (c === '(') return parseList();
    if (c === '"') return parseString();
    let start = i;
    while (i < s.length && s[i] !== ' ' && s[i] !== '(' && s[i] !== ')') i++;
    const atom = s.slice(start, i);
    return (atom === 'NIL' || atom === '') ? null : atom;
  }
  skipWs();
  return parseValue();
}

/** IANA charset → Node Buffer 인코딩(미지원은 utf8 best-effort). EUC-KR 등 멀티바이트 레거시는 미지원. */
function charsetToNode(cs) {
  const c = String(cs || '').toLowerCase();
  if (c === 'utf-8' || c === 'utf8') return 'utf8';
  if (c === 'us-ascii' || c === 'ascii') return 'ascii';
  if (c === 'iso-8859-1' || c === 'latin1') return 'latin1';
  return 'utf8';
}

/**
 * [메일 인코딩] charset 라벨을 WHATWG TextDecoder 라벨로 정규화. EUC-KR(ks_c_5601-1987·CP949 등)을
 *   포함한 레거시 멀티바이트를 지원한다(한국·일본·중국 메일). 미지정/공백은 utf-8.
 */
function normalizeCharset(cs) {
  const c = String(cs || '').toLowerCase().trim().replace(/^["']|["']$/g, '');
  if (!c) return 'utf-8';
  if (c === 'ks_c_5601-1987' || c === 'ksc5601' || c === 'ksc_5601' || c === 'ks_c_5601' || c === 'korean' || c === 'cp949' || c === 'ms949' || c === 'uhc' || c === 'x-windows-949') return 'euc-kr';
  if (c === 'utf8') return 'utf-8';
  if (c === 'latin1') return 'iso-8859-1';
  if (c === 'cp932' || c === 'ms932' || c === 'x-sjis') return 'shift_jis';
  return c; // utf-8·euc-kr·iso-8859-*·windows-125x·shift_jis·euc-jp·gb2312·gbk·big5 등은 TextDecoder가 처리
}

/**
 * [메일 인코딩] 바이트(Buffer/latin1문자열)를 charset에 맞게 유니코드 문자열로 디코드.
 *   Node가 번들한 ICU 기반 TextDecoder를 써 EUC-KR 등 레거시를 정확히 디코드한다(Buffer.toString은 미지원).
 *   미지원 라벨·실패 시 utf-8 best-effort.
 * @param {Buffer|string} input Buffer 또는 latin1(바이트 보존) 문자열
 * @param {string} charset
 * @returns {string}
 */
function decodeCharset(input, charset) {
  const bytes = Buffer.isBuffer(input) ? input : Buffer.from(String(input == null ? '' : input), 'latin1');
  const label = normalizeCharset(charset);
  try { return new TextDecoder(label, { fatal: false }).decode(bytes); }
  catch (_) {
    try { return new TextDecoder('utf-8', { fatal: false }).decode(bytes); }
    catch (__) { return bytes.toString('utf8'); }
  }
}

/**
 * RFC2047 인코딩 헤더(=?charset?B/Q?text?=)를 디코드한다. 연속 인코딩워드 사이 공백은 제거.
 *   B=base64, Q=quoted-printable(_=공백). 미지원 charset은 utf8 best-effort. Buffer 사용(Node).
 */
function decodeMimeHeader(s) {
  if (typeof s !== 'string' || !s) return (typeof s === 'string') ? s : null;
  // 인접한 인코딩워드 사이의 공백류 제거(RFC2047 §6.2).
  const collapsed = s.replace(/\?=\s+=\?/g, '?==?');
  return collapsed.replace(/=\?([^?]+)\?([BbQq])\?([^?]*)\?=/g, (m, charset, enc, text) => {
    try {
      let bytes;
      if (enc.toUpperCase() === 'B') {
        bytes = Buffer.from(text, 'base64');
      } else {
        const q = text.replace(/_/g, ' ').replace(/=([0-9A-Fa-f]{2})/g, (mm, h) => String.fromCharCode(parseInt(h, 16)));
        bytes = Buffer.from(q, 'latin1');
      }
      return decodeCharset(bytes, charset); // [메일 인코딩] EUC-KR 등 레거시 charset 정확히 디코드

    } catch (_) {
      return m; // 디코드 실패 시 원문 유지
    }
  });
}

/** ENVELOPE 주소 구조 [[name, adl, mailbox, host], ...] → 표시 문자열(이름 우선, 없으면 mailbox@host). */
function formatEnvelopeAddr(addrList) {
  if (!Array.isArray(addrList) || addrList.length === 0) return null;
  const a = addrList[0];
  if (!Array.isArray(a)) return null;
  const name = a[0], mailbox = a[2], host = a[3];
  if (typeof name === 'string' && name) return decodeMimeHeader(name);
  if (mailbox && host) return mailbox + '@' + host;
  return (typeof mailbox === 'string' && mailbox) ? mailbox : null;
}

/**
 * FETCH 응답 라인에서 ENVELOPE를 추출한다(리터럴은 리더가 따옴표로 정규화한 뒤 호출).
 *   ENVELOPE 필드 순서: [date, subject, from, sender, reply-to, to, cc, bcc, in-reply-to, message-id]
 * @returns {{uid:number|null, date:string|null, subject:string|null, from:string|null}|null}
 */
function parseFetchEnvelope(line) {
  if (typeof line !== 'string') return null;
  const idx = line.search(/ENVELOPE\s*\(/i);
  if (idx < 0) return null;
  const open = line.indexOf('(', idx);
  if (open < 0) return null;
  const env = parseSexp(line.slice(open));
  if (!Array.isArray(env)) return null;
  const um = line.match(/\bUID\s+(\d+)/i);
  return {
    uid: um ? Number(um[1]) : null,
    date: (typeof env[0] === 'string') ? env[0] : null,
    subject: decodeMimeHeader(env[1]),
    from: formatEnvelopeAddr(env[2]),
  };
}

/**
 * FETCH 응답 라인에서 FLAGS를 추출한다(읽음/삭제 상태 동기화용). 리터럴은 리더가 정규화한 뒤 호출.
 *   `* 3 FETCH (UID 12 FLAGS (\Seen \Answered))` → { uid:12, seen:true, deleted:false }
 *   FLAGS 부재 시 null.
 * @returns {{uid:(number|null), seen:boolean, deleted:boolean}|null}
 */
function parseFetchFlags(line) {
  if (typeof line !== 'string') return null;
  const fm = line.match(/FLAGS\s*\(([^)]*)\)/i);
  if (!fm) return null;
  const flags = fm[1].trim().toLowerCase().split(/\s+/).filter(Boolean);
  const um = line.match(/\bUID\s+(\d+)/i);
  return {
    uid: um ? Number(um[1]) : null,
    seen: flags.includes('\\seen'),
    deleted: flags.includes('\\deleted'),
  };
}

/**
 * IMAP LIST/LSUB 응답 한 줄을 파싱한다(메일함 목록).
 *   `* LIST (\HasNoChildren \Trash) "/" "[Gmail]/Trash"`
 *     → { flags:['\\HasNoChildren','\\Trash'], delimiter:'/', name:'[Gmail]/Trash' }
 *   리터럴({n}) 메일함명은 리더(_tryTakeLine)가 따옴표 문자열로 정규화한 뒤 호출된다고 가정한다.
 *   파싱 전략: `* LIST ` 이후를 괄호로 감싸 parseSexp로 [flags, delimiter, name] 형태로 읽는다.
 * @returns {{flags:string[], delimiter:(string|null), name:string}|null} 비-LIST/형식 불일치면 null.
 */
function parseListMailbox(line) {
  if (typeof line !== 'string') return null;
  const m = line.match(/^\*\s+(?:LIST|LSUB)\s+/i);
  if (!m) return null;
  const parsed = parseSexp('(' + line.slice(m[0].length) + ')');
  if (!Array.isArray(parsed) || parsed.length < 3) return null;
  const flags = Array.isArray(parsed[0]) ? parsed[0].filter((f) => typeof f === 'string') : [];
  const delimiter = (typeof parsed[1] === 'string') ? parsed[1] : null;
  // 메일함명은 마지막 토큰(따옴표/리터럴 정규화로 단일 문자열). 잘못된 형식이면 null.
  const name = (typeof parsed[parsed.length - 1] === 'string') ? parsed[parsed.length - 1] : null;
  if (!name) return null;
  return { flags, delimiter, name };
}

// 수집 제외 메일함. SPECIAL-USE 플래그(RFC6154)를 우선하고, 플래그 미제공 서버를 위해 이름(잎)으로도 차단한다.
//   \Trash·\Junk: 휴지통/스팸(사용자 요청 제외). \All: Gmail "전체보관함"=상위집합이라 폴더 순회와 중복.
//   \Drafts: 임시보관(수신 메일 아님). \Noselect/\NonExistent: 선택 불가(컨테이너 노드).
const EXCLUDED_MAILBOX_FLAGS = new Set(['\\noselect', '\\nonexistent', '\\trash', '\\junk', '\\all', '\\drafts']);
const EXCLUDED_MAILBOX_LEAVES = new Set([
  'trash', 'deleted', 'deleted messages', 'deleted items', 'bin',
  'junk', 'junk e-mail', 'junk email', 'spam', 'bulk', 'bulk mail',
  'drafts',
  '휴지통', '지운편지함', '지운 편지함', '삭제된 편지함',
  '스팸', '스팸편지함', '스팸 편지함', '정크',
  '임시보관함', '임시 보관함',
  'all mail', '전체보관함', '모든 메일', '모든메일',
]);

/**
 * IMAP modified UTF-7(RFC 3501 §5.1.3) 메일함명을 유니코드로 디코드한다(한글 등 표시용).
 *   · '&' 다음 '-'까지의 구간은 modified base64(',' → '/')로 인코딩된 UTF-16BE.
 *   · '&-' → 리터럴 '&'. '&'가 없으면 ASCII이므로 그대로 반환.
 *   주의: 이 값은 **표시 전용**이다. IMAP 명령(EXAMINE 등)에는 원본(인코딩된) 이름을 그대로 써야 한다.
 * @param {string} s 원본(modified UTF-7) 메일함명
 * @returns {string} 디코드된 표시 문자열
 */
function decodeModifiedUtf7(s) {
  if (typeof s !== 'string' || s.indexOf('&') === -1) return (typeof s === 'string') ? s : '';
  let out = '';
  let i = 0;
  while (i < s.length) {
    if (s[i] !== '&') { out += s[i]; i++; continue; }
    const end = s.indexOf('-', i + 1);
    if (end === -1) { out += s.slice(i); break; } // 닫힘 없음 — 원문 보존
    const b64 = s.slice(i + 1, end);
    if (b64 === '') { out += '&'; } // '&-' → '&'
    else {
      try {
        const buf = Buffer.from(b64.replace(/,/g, '/'), 'base64'); // UTF-16BE 바이트
        let str = '';
        for (let j = 0; j + 1 < buf.length; j += 2) str += String.fromCharCode((buf[j] << 8) | buf[j + 1]);
        out += str || s.slice(i, end + 1);
      } catch (_) { out += s.slice(i, end + 1); }
    }
    i = end + 1;
  }
  return out;
}

/**
 * 메일 수집 대상 메일함인지 판단한다(휴지통/스팸/전체보관함/임시보관함/선택불가 제외).
 *   SPECIAL-USE 플래그를 우선하고, 플래그가 없는 서버를 위해 메일함명의 잎(leaf)으로도 판단한다.
 * @param {{flags?:string[], delimiter?:(string|null), name?:string}} mb
 * @returns {boolean}
 */
function isCollectibleMailbox(mb) {
  if (!mb || typeof mb.name !== 'string' || !mb.name) return false;
  const flags = Array.isArray(mb.flags) ? mb.flags : [];
  for (const f of flags) { if (EXCLUDED_MAILBOX_FLAGS.has(String(f).toLowerCase())) return false; }
  const delim = (typeof mb.delimiter === 'string' && mb.delimiter) ? mb.delimiter : '/';
  const parts = mb.name.split(delim);
  const leaf = parts[parts.length - 1].toLowerCase().trim();
  return !EXCLUDED_MAILBOX_LEAVES.has(leaf);
}

module.exports = {
  imapQuote, parseTaggedLine, parseStatusItems,
  parseSearchUids, parseSexp, decodeMimeHeader, formatEnvelopeAddr, parseFetchEnvelope, charsetToNode,
  normalizeCharset, decodeCharset,
  parseListMailbox, isCollectibleMailbox, parseFetchFlags, decodeModifiedUtf7,
};
