...
const { imapQuote, parseTaggedLine, parseStatusItems, parseSearchUids, parseFetchEnvelope } = require('./imapProtocol');
...
class ImapClient {
  ...
  /**
   * 버퍼에서 논리 라인 1개를 추출한다(리터럴 {n} 인지). 리터럴은 따옴표 문자열로 정규화해 인라인
  ...
  _tryTakeLine() {
    ...
    while (true) {
      ...
      const chunk = this._buf.slice(0, nl);
      const m = chunk.match(/\{(\d+)\}$/); // 끝이 리터럴 마커({n})?
      if (!m) {
        this._buf = this._buf.slice(nl + 2);
        return out + chunk; // 논리 라인 완성
      }
      const n = parseInt(m[1], 10);
      const litStart = nl + 2;
      if (this._buf.length < litStart + n) return null; // 리터럴 전체 미수신
      const literal = this._buf.slice(litStart, litStart + n);
      out += chunk.slice(0, chunk.length - m[0].length) + imapQuote(literal); // {n} → "literal"
      this._buf = this._buf.slice(litStart + n); // 리터럴까지 소비하고 같은 논리 라인 계속
    }
  }
