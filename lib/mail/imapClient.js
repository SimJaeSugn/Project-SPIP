...
const { imapQuote, parseTaggedLine, parseStatusItems, parseSearchUids, parseFetchEnvelope, parseSexp, parseListMailbox, isCollectibleMailbox } = require('./imapProtocol');
...
class ImapClient {
  ...
  async fetchMessage(uid, mailbox, maxBytes) {
    ...
    try {
      ...
      const bytes = await this._fetchBodyRaw(u, n); // Buffer(원시 바이트)
  ...
  /**
   * [메일 인코딩] BODY.PEEK[] 응답의 본문 리터럴을 **원시 바이트 Buffer**로 수집(UTF-8 디코드 안 함).
  ...
  _fetchBodyRaw(uid, n) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        this._sock.write(tag + ' UID FETCH ' + uid + ' BODY.PEEK[]<0.' + n + '>\r\n');
      } catch (err) {
