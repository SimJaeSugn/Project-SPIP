...
const briefingConnRegistry = require('../ai/briefingConnections');

const FILE_MODE = 0o600;

// [M13 M-1] briefing baseURL 검증 상한.
const MAX_BRIEFING_URL_LEN = 2048;
// [SH-1] 셸프 URL 등 일반 http(s) URL 검증 기본 길이 상한.
const MAX_HTTP_URL_LEN = 2048;
const MAX_BRIEFING_MODEL_LEN = 200;
const MAX_BRIEFING_KEY_LEN = 4096;
// 사용자 편집 System(지시) 프롬프트 상한(자). 빈 값('')이면 시드(briefingPrompt.DEFAULT_SYSTEM_PROMPT) 사용.
const MAX_BRIEFING_SYSTEM_PROMPT_LEN = 8000;
const LOCALHOST_HOSTS = new Set(['127.0.0.1', 'localhost', '::1', '[::1]']);

// 내장 기본값 (§8.2)
const DEFAULTS = Object.freeze({
  scanRoots: [],
  // [탐색기 위젯] 열람 전용 루트(스캔 대상 아님). dialog로 명시 등록한 폴더만 들어온다(EXP-H-1).
  explorerRoots: [],
  ...
  // [SH-4] 즐겨찾기 셸프 URL 자동 재크롤(6시간) 토글. 기본 ON(PM #1) — 사용자가 끌 수 있다.
  //   off면 스케줄러 tick이 egress 0(D-SCHED-1). 백그라운드 외부 접속이라 명시 토글 제공.
  shelfAutoRefresh: true,
  ...
  briefing: Object.freeze({
    ...
    apiKey: '',
    // 사용자 편집 System(지시) 프롬프트. 빈 값('')이면 시드(briefingPrompt 기본값)를 사용한다.
    //   신뢰 영역이나 normalizeBriefing이 길이상한·제어문자 정제를 적용한다(시크릿 아님·노출 가능).
    systemPrompt: '',
    temperature: 0.3,
    maxTokens: 1024,
    timeoutMs: 30000,
  ...
  // [AI 연결 복수화] 저장된 AI 연결 목록 + 활성 연결 id. briefing 은 항상 활성 연결의 실효 미러다
  //   (briefingConnections.resolveState 가 불변식 강제·레거시 이행). 기본 빈 배열 → 로드 시 briefing 에서
  //   첫 연결로 승격(무손실). apiKey 는 엔트리에 평문 0600 저장(렌더러 비노출).
  briefingConnections: Object.freeze([]),
  activeBriefingId: '',
});
...
const LIMITS = Object.freeze({
  ...
  maxScanRoots: 64,
  // [탐색기 위젯] 열람 루트 수 상한 — dialog 로 하나씩 등록하는 성격이라 낮게 잡는다.
  maxExplorerRoots: 16,
...
/**
 * [탐색기 위젯] explorerRoots 정규화 — 사용자가 dialog 로 명시 등록한 **열람 전용** 루트.
 *   scanRoots(스캔 대상)와 직교한다: 스캔하지 않고, 프로젝트로 취급하지도 않는다.
 *   여기서는 실경로화·디렉터리 여부·중복·개수 상한만 본다. 민감/시스템 경로 deny 게이트는
 *   **사용 시점**(browsePolicy.gate)이 단일 신뢰 경계 — 등록 후 경로 의미가 바뀌어도 그때 거부된다.
 * @param {*} input
 * @param {object} logger
 * @returns {string[]} canonicalize된 실경로 배열
 */
function normalizeExplorerRoots(input, logger) {
  if (!Array.isArray(input)) {
    if (input !== undefined) logger.warn('explorerRoots가 배열이 아니어서 무시합니다');
    return [];
  }
  const out = [];
  const seen = new Set();
  for (const item of input.slice(0, LIMITS.maxExplorerRoots)) {
    if (typeof item !== 'string' || !item.trim()) continue;
    const real = pathGuard.canonicalize(item);
    if (real === null) {
      logger.warn('explorerRoots 경로를 해소하지 못해 건너뜁니다');
      continue;
    }
    try {
...
/**
 * [SH-1] 일반 http(s) URL 검증(M-1 일반화). 다음을 강제:
 *   ① new URL() 파싱 성공
 *   ② 임베디드 자격증명(http://user:pass@host) 거부 — username/password 비어야 함
 *   ③ scheme ∈ {http,https}만 (file:·ftp:·javascript:·기타 차단)
 *   ④ 길이 상한(opts.maxLen, 기본 2048)
 * 셸프 URL 북마크·브리핑 baseURL 등이 공유하는 단일 검증 경계.
 * @param {*} input
 * @param {object} [opts] { maxLen }
 * @returns {{ ok:boolean, value:string|null, external:boolean }}
 *   ok=false면 불량(호출처가 거부/폴백). external=비-localhost host(설정 UI 경고용).
 */
function validateHttpUrl(input, opts) {
  opts = opts || {};
  const maxLen = (typeof opts.maxLen === 'number' && opts.maxLen > 0) ? opts.maxLen : MAX_HTTP_URL_LEN;
  if (typeof input !== 'string' || !input || input.length > maxLen) {
...
/**
 * [M13 M-1] briefing baseURL 검증 — validateHttpUrl의 별칭/래퍼(반환형·시그니처 불변).
 *   기존 briefing 호출 회귀 0을 위해 보존한다(길이 상한은 briefing 전용 상수).
 * @param {*} input
 * @returns {{ ok:boolean, value:string|null, external:boolean }}
 */
function validateBriefingUrl(input) {
  return validateHttpUrl(input, { maxLen: MAX_BRIEFING_URL_LEN });
}

/**
 * 사용자 편집 System(지시) 프롬프트 정제. 신뢰 영역이나 길이상한·제어문자 정제는 적용.
 *   줄바꿈(\n)·탭(\t)은 다행 프롬프트 구조에 필요해 보존, 나머지 C0/DEL·방향성 제어는 제거.
 *   빈 값·비문자열·정제 후 공백뿐이면 ''(빈=시드 사용). briefingPrompt.sanitizeSystemPrompt와 동일 정책.
 * @param {*} v
 * @returns {string}
 */
function sanitizeSystemPrompt(v) {
  ...
  const limited = cleaned.length > MAX_BRIEFING_SYSTEM_PROMPT_LEN ? cleaned.slice(0, MAX_BRIEFING_SYSTEM_PROMPT_LEN) : cleaned;
...
function normalizeBriefing(input, logger) {
  ...
  const systemPrompt = sanitizeSystemPrompt(src.systemPrompt);
...
function loadConfig(options) {
  ...
  const config = {
    ...
    // [탐색기 위젯] 열람 루트(스캔 무관). 사용 시점 deny 게이트는 browsePolicy.gate.
    explorerRoots: normalizeExplorerRoots(merged.explorerRoots, logger),
    ...
    // [SH-4] 셸프 자동 재크롤 토글(기본 true).
    shelfAutoRefresh: validateBoolean(merged.shelfAutoRefresh, DEFAULTS.shelfAutoRefresh),
    // [M4 R-09] size 수집 설정.
  ...
  // [AI 연결 복수화] 연결 목록·활성 id 정규화 + 활성↔briefing 미러 불변식 확립.
  //   최초/레거시(목록 없음)면 위의 briefing 을 첫 연결로 승격(무손실). 이후 briefing 은 활성 연결 미러다.
  const connState = briefingConnRegistry.resolveState(
    config.briefing, merged.briefingConnections, merged.activeBriefingId,
    { normalizeSettings: (inp) => normalizeBriefing(inp, logger) },
  );
  config.briefing = connState.briefing;
  config.briefingConnections = connState.connections;
  config.activeBriefingId = connState.activeId;
...
module.exports = {
  ...
  persistConfigKeys,
  normalizeScanRoots,
  normalizeExplorerRoots,
  normalizeExcludes,
  normalizeDetectSignals,
  normalizeSize,
  normalizeScan,
  normalizeBriefing,
  briefingConnRegistry,
