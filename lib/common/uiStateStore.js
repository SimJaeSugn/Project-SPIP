...
// [로드맵 Phase 3·G] 스크래치패드 메모(scratchpad)·통합 커밋 히트맵(commitHeatmap) 위젯 추가 — featureAdd 앞.
//   app.js HOME_SECTION_IDS와 동형(드리프트 테스트).
...
const NEW_HIDDEN_SINCE = [
  ...
  { since: 3, ids: ['scratchpad', 'commitHeatmap'] },  // v3: 스크래치패드 메모 · 통합 커밋 히트맵
];
...
/** [Phase 5 예약·프리폼] 위젯 좌표 맵 정규화 — { [id]: {x,y} }. 토글 위젯 화이트리스트만, x/y 정수 [0,MAX_POS]. */
...
function normalizePreset(input, fallbackId) {
  ...
  return {
    ...
    layoutMode,
    groups: [], // [Phase 5 예약] 그룹/섹션 노드 — 스키마 확정 전까지 빈 배열로 정규화
  };
}
