...
const MAX_TODOS = 200; // 할 일 항목 수 상한(박스 1개당)
const MAX_TODO_LEN = 500; // 할 일 텍스트 길이 상한
const MAX_TODO_BOXES = 48; // [위젯 인스턴스] 할 일 박스(=todos 위젯 인스턴스) 수 상한 — MAX_WIDGETS 와 동급(무한 증식 방어)
...
function normalizeState(obj) {
  ...
  // [위젯 인스턴스] 전역 todos 는 폐기(deprecated) — 이제 할 일은 todoBoxes(iid별)와 legacyTodos(흡수 대기)에만 산다.
  //   호환을 위해 응답 키(todos)는 유지하되 항상 빈 배열이다(단일 진실은 아래 todoBoxes/legacyTodos).
  const todos = [];
  ...
  // [위젯 인스턴스 · High-1] 할 일도 인스턴스별 박스 { iid: Todo[] } — 배치된 iid 로 게이트(removeWidget 시 자동 정리).
  //   ⚠️ 게이트를 '활성 프리셋의 iid'로만 잡으면 비활성 프리셋에만 있는 todos 위젯의 박스가 프리셋 전환
  //   (write→normalizeState)에서 통째로 삭제·영구 손실된다(sizes 는 프리셋별로 저장돼 무사하지만 todoBoxes 는
  //   최상위 단일 맵이라 위험). 그래서 allowed = **활성 homeWidgets ∪ 전(全) 프리셋 위젯 iid**의 합집합으로 잡는다:
  //     · 어느 프리셋에도 없는 iid(=진짜 삭제된 위젯)만 정리 → removeWidget 시 데이터 삭제 유지(요구 충족).
  //     · 다른 프리셋에 아직 존재하는 iid 는 보존 → 프리셋 전환 손실 제거.
  //   프리셋이 없으면(단일 레이아웃) 활성 placed 와 동일하다. 전역 todos 는 legacyTodos 로 보존됐다가 첫 할 일
  //   위젯이 흡수한다(ipc/uiState.adoptLegacyTodos).
  const todoAllowed = allPresetWidgetIids(dashboard, placed);
  const todoBoxes = normalizeTodoBoxes(obj.todoBoxes, todoAllowed);
  const legacyTodos = migrateLegacyTodos(obj);
  return { schemaVersion: SCHEMA_VERSION, favorites, order, sortMode, names, theme, accent, uiScale, todos, todoBoxes, legacyTodos, langTrend, homeWidgets, homeWidgetSizes, briefing, aiUsage, shelfBookmarks, scratchpads, dashboard };
}
...
/**
 * [위젯 인스턴스] 할 일은 이제 **인스턴스별 목록**이다 — { [iid]: Todo[] }.
 *   할 일 위젯을 2개 놓으면 서로 다른 목록을 쓴다(같은 목록을 두 번 보여주는 건 중복 배치의 의미가 없다).
 *   mdDocStore.normalizeBoxes(문서함) 동형: 키(iid) 형식 화이트리스트 + 박스 수·박스당 항목 수 상한,
 *   항목 정규화(normalizeTodos)로 손상/중복 id/빈 텍스트 폐기. 이 함수가 유일 검증 경계다.
 *
 * [고아 키 정리] scratchpads(콘텐츠라 보존)와 달리, 할 일 박스는 크기·좌표·그룹과 같이 **배치된 iid 집합**
 *   으로 게이트한다 — 배치에서 사라진 위젯의 박스는 여기서 제거된다(removeWidget → write → normalizeState
 *   흐름에서 삭제된 위젯의 할 일이 자동 정리). 단 legacy 흡수(adoptLegacyTodos) 전 전역 todos 는 별도 필드
 *   (legacyTodos)로 보존되므로 이 게이트가 흡수 전 데이터를 날리지 않는다.
 *   [High-1] normalizeState 는 allowed 에 **활성 + 전(全) 프리셋 위젯 iid 의 합집합**(allPresetWidgetIids)을
 *   넘긴다 — 비활성 프리셋에만 있는 박스가 프리셋 전환에서 삭제되지 않게. allowed 미전달 시 iid 형식만
 *   검사(순서 의존 회피용 폴백 — 이땐 고아 정리 없음).
 *
 * [T-2 · DoS 방어선] 박스 폭증 방어는 3중이다: ① 키 개수 상한 MAX_TODO_BOXES(48), ② 배치 iid 게이트(allowed —
 *   미배치 iid 는 애초에 못 심음), ③ **최종 방어선 = read 단계의 MAX_UISTATE_BYTES(1MB)**. 정규화 상한을
 *   통과해도 직렬화 결과가 1MB 를 넘으면 read 가 파싱 전 기본 상태로 폴백하므로(uiStateStore.read ①),
 *   손상/공격 입력이 무한히 커질 수 없다.
 * @param {*} input
 * @param {Set<string>} [allowed] 배치된(허용) iid 집합
 * @returns {Object<string, object[]>}
 */
function normalizeTodoBoxes(input, allowed) {
  const out = {};
  if (!isPlainObject(input)) return out;
  let n = 0;
  for (const iid of Object.keys(input)) {
    if (n >= MAX_TODO_BOXES) break;
    if (!IID_RE.test(iid)) continue;              // 키(iid) 형식 화이트리스트
    if (allowed && !allowed.has(iid)) continue;    // 고아 키(미배치 위젯) 제거 — 배치된 iid 만
    out[iid] = normalizeTodos(input[iid]);
    n++;
  }
  return out;
}
...
/**
 * [High-1] 전(全) 프리셋에 배치된 위젯 인스턴스 iid 의 합집합 — 최상위 단일 맵(todoBoxes)의 고아 게이트 소스.
 *   활성 프리셋뿐 아니라 모든 프리셋(dashboard.presets[*].widgets)의 iid 를 모아, 다른 프리셋에만 있는
 *   todos 위젯의 박스가 프리셋 전환 시 삭제되지 않게 한다. base(활성 homeWidgets iid)도 함께 union 하여
 *   dashboard 정규화 이전/이후 순서와 무관하게 안전(단일 레이아웃이면 base 만).
 * @param {object} dashboard 정규화된 대시보드 상태({presets:[{widgets:[{iid}]}]})
 * @param {Set<string>} [base] 활성 homeWidgets iid 집합
 * @returns {Set<string>}
 */
function allPresetWidgetIids(dashboard, base) {
  const out = new Set(base instanceof Set ? base : []);
  const presets = (dashboard && Array.isArray(dashboard.presets)) ? dashboard.presets : [];
  for (const p of presets) {
    const ws = (p && Array.isArray(p.widgets)) ? p.widgets : [];
    for (const w of ws) if (w && typeof w.iid === 'string') out.add(w.iid);
  }
  return out;
}

/**
 * [Med-1] 박스(iid)의 할 일 목록 — 없으면 빈 배열. mdDocStore.docsOf 동형(저장소가 단일 경계로 소유).
 *   context.js·agent.js·ipc/uiState.js 가 "박스에서 목록 꺼내기"를 각자 인라인하지 않고 이걸 재사용한다.
 * @param {object} state 정규화된(또는 부분) 상태
 * @param {string} box iid
 * @returns {object[]}
 */
function todosOf(state, box) {
  const boxes = (state && isPlainObject(state.todoBoxes)) ? state.todoBoxes : {};
  return Array.isArray(boxes[box]) ? boxes[box] : [];
}

/**
 * [Med-1] 박스(iid)만 교체한 새 todoBoxes 맵(불변) — 다른 인스턴스의 박스는 그대로. mdDocStore.withDocs 동형.
 *   호출부는 이 결과를 state.todoBoxes 로 실어 write 한다(정규화가 최종 검증).
 * @param {object} state
 * @param {string} box iid
 * @param {object[]} todos
 * @returns {Object<string, object[]>}
 */
function withTodos(state, box, todos) {
  return Object.assign({}, (state && state.todoBoxes) || {}, { [box]: todos });
}

/**
 * [v(구) 전역 todos → 인스턴스 박스 이행] 전역 todos 배열은 아직 어느 인스턴스의 것도 아니다 →
 *   legacyTodos 로 보존한다(첫 할 일 위젯 인스턴스가 adoptLegacyTodos 로 흡수). mdDocStore.normalizeState
 *   의 legacy 처리와 동형. 이미 todoBoxes 가 있으면(신형) 전역 todos 는 없다고 본다.
 * @param {object} obj 저장 상태
 * @returns {object[]} 흡수 대기 중인 레거시 할 일 배열(정규화됨)
 */
function migrateLegacyTodos(obj) {
  // mdDocStore.normalizeState 동형: 최상위 전역 todos 가 있으면 그것이 아직 흡수 안 된 레거시다(우선).
  //   전역 todos 는 정규화 출력에서 항상 빈 배열이 되므로(단일 진실 = boxes/legacy), 재적재된 파일의
  //   todos 는 [] 라 이 분기는 재흡수를 일으키지 않는다. 손으로 만든 파일에서 boxes+todos 공존 시에도 무손실.
  const fromGlobal = normalizeTodos(obj.todos);
  return fromGlobal.length > 0 ? fromGlobal : normalizeTodos(obj.legacyTodos);
}
...
module.exports = {
  ...
  normalizeNames,
  normalizeTodos,
  // [위젯 인스턴스] 인스턴스별 할 일 박스 { iid: Todo[] } + 전역 todos 흡수 대기 이행.
  normalizeTodoBoxes,
  migrateLegacyTodos,
  // [Med-1] 박스 목록 조회/교체 단일 헬퍼(mdDocStore.docsOf/withDocs 동형) — ipc/context/agent 재사용.
  todosOf,
  withTodos,
  // [High-1] 전 프리셋 위젯 iid 합집합(todoBoxes 고아 게이트 소스).
  allPresetWidgetIids,
  MAX_TODO_BOXES,
  sanitizeTodoText,
  normalizeLangCounts,
