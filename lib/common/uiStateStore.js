...
//   v2: 셸프(shelf/shelfWide). v3: 스크래치패드 메모(scratchpad). NEW_HIDDEN_SINCE 단일 출처.
 ...
 *   형태: { [id]: { w, h } }. id 는 토글 가능 위젯(featureAdd 제외) 화이트리스트만 허용.
 *   w = [1, HOME_MAX_COLS] 정수(열 스팬). h = [HOME_H_MIN, HOME_H_MAX] 정수 또는 null(자동 높이).
 *   손상/미지 항목은 제거. homeLayout(순서)·hiddenWidgets(표시)와 직교 — 리사이즈만 관장.
 * @param {*} input
 * @returns {Object<string,{w:number,h:(number|null)}>}
 */
function normalizeHomeWidgetSizes(input) {
  const out = {};
  if (!isPlainObject(input)) return out;
  for (const id of Object.keys(input)) {
    // [로드맵 Phase 5·M] 그룹 id(GROUP_ID_RE)도 허용 — 프리폼에서 그룹 블록 폭 조절. 그 외 미지 id 제거.
    if (!TOGGLEABLE_WIDGET_SET.has(id) && !GROUP_ID_RE.test(id)) continue;
    const v = input[id];
...
// [로드맵 Phase 5·M] 그룹/섹션 — { id, name, collapsed, members[] }. 위젯은 한 그룹에만 소속.
const GROUP_ID_RE = /^g[0-9a-z]{4,32}$/; // 'g'+영숫자(렌더러 생성, main 검증)
const MAX_GROUPS = 12;             // 프리셋당 그룹 상한
const MAX_GROUP_NAME = 60;

/** [로드맵 Phase 5·M] 그룹 배열 정규화(순수·신뢰 경계) — id 형식·중복 제거·이름 sanitize·members(토글 위젯,
 *   그룹 간 유일·그룹 내 중복 제거)·collapsed bool·개수 상한. 손상/비배열 graceful [] . */
function normalizeGroups(input) {
  if (!Array.isArray(input)) return [];
  const out = [];
  const seenIds = new Set();
  const claimed = new Set(); // 한 위젯은 한 그룹에만
  for (const g of input) {
    ...
    if (Array.isArray(g.members)) {
      for (const m of g.members) {
        if (TOGGLEABLE_WIDGET_SET.has(m) && !claimed.has(m) && members.indexOf(m) < 0) { members.push(m); claimed.add(m); }
      }
    }
    out.push({ id, name, collapsed, members });
  }
  return out;
}

/** [로드맵 Phase 5·B] 위젯 좌표 맵 정규화 — { [id]: {x,y} }. 홈 섹션 화이트리스트(featureAdd 포함 — 프리폼에서
 *   추가 카드도 자유 배치), x/y 정수 [0,MAX_POS]. */
function normalizeWidgetPositions(input) {
  const out = {};
  if (!isPlainObject(input)) return out;
  for (const id of Object.keys(input)) {
    // [로드맵 Phase 5·M] 홈 섹션(featureAdd 포함) + 그룹 id(GROUP_ID_RE) 허용 — 프리폼에서 그룹 블록도 자유 배치.
    if (!HOME_SECTION_SET.has(id) && !GROUP_ID_RE.test(id)) continue;
    const v = input[id];
    if (!isPlainObject(v)) continue;
...
/**
 * 단일 프리셋 정규화 — id(슬러그)·name·layout·hidden·sizes·positions·layoutMode·groups.
 *   layout/hidden/sizes 는 기존 단일 신뢰 경계 재사용(정규화 출처 단일화). positions 는 프리폼 전용,
 *   groups 는 Phase 5(그룹/섹션) 예약이라 스키마 확정 전까지 항상 [] 로 정규화한다.
 * @param {*} input 프리셋 후보
...
function normalizePreset(input, fallbackId) {
  ...
  return {
    id,
    name,
    layout: normalizeHomeLayout(o.layout),
    hidden: normalizeHiddenWidgets(o.hidden),
    sizes: normalizeHomeWidgetSizes(o.sizes),
    positions: normalizeWidgetPositions(o.positions),
    layoutMode,
    groups: normalizeGroups(o.groups), // [로드맵 Phase 5·M] 그룹/섹션 노드
  };
}

/** 기본 프리셋 1개(현행 기본 배치)로 구성된 기본 대시보드 상태. */
function defaultDashboardState() {
  return {
    ...
    presets: [{
      ...
      hidden: DEFAULT_HIDDEN_WIDGETS.slice(),
      sizes: {},
      positions: {},
      layoutMode: 'masonry',
      groups: [],
 ...
 */
function normalizeDashboardState(input) {
  ...
  for (let i = 0; i < input.presets.length; i++) {
    ...
    const p = normalizePreset(input.presets[i], 'p' + i);
 ...
 *   legacy = { homeLayout, hiddenWidgets, homeWidgetSizes }(정규화 여부 무관 — 내부에서 재정규화).
 *   기존 사용자의 배치·숨김·크기를 '기본' 프리셋으로 승격한다(정보 손실 0).
 * @param {*} legacy
 * @returns {{schemaVersion:number,activePreset:string,presets:Array}}
 */
function migrateLegacyToDashboard(legacy) {
  ...
  return {
    ...
    presets: [{
      id: DEFAULT_PRESET_ID, name: '기본',
      layout: normalizeHomeLayout(o.homeLayout),
      hidden: normalizeHiddenWidgets(o.hiddenWidgets),
      sizes: normalizeHomeWidgetSizes(o.homeWidgetSizes),
      positions: {},
      layoutMode: 'masonry',
      groups: [],
    }],
...
function presetAdd(state, name) {
  ...
  const preset = { id, name: sanitizeBriefingText(name, MAX_PRESET_NAME) || '새 대시보드', layout: HOME_SECTION_IDS.slice(), hidden: DEFAULT_HIDDEN_WIDGETS.slice(), sizes: {}, positions: {}, layoutMode: 'masonry', groups: [] };
...
/** 프리셋 내용 갱신(렌더러 편집 영속용) — layout/hidden/sizes/positions/layoutMode 부분 패치(정규화). */
function presetUpdate(state, id, patch) {
  const s = normalizeDashboardState(state);
  const p0 = isPlainObject(patch) ? patch : {};
  const presets = s.presets.map((p) => {
    if (p.id !== id) return p;
    return normalizePreset(Object.assign({}, p, {
      layout: (p0.layout !== undefined) ? p0.layout : p.layout,
      hidden: (p0.hidden !== undefined) ? p0.hidden : p.hidden,
...
function defaultState() {
  ...
  return { schemaVersion: SCHEMA_VERSION, favorites: [], order: [], sortMode: 'auto', names: {}, theme: 'system', todos: [], langTrend: { generatedAt: null, prev: {}, cur: {} }, homeLayout: HOME_SECTION_IDS.slice(), hiddenWidgets: DEFAULT_HIDDEN_WIDGETS.slice(), homeWidgetSizes: {}, briefing: defaultBriefing(), aiUsage: defaultAiUsage(), shelfBookmarks: [], scratchpad: defaultScratchpad(), dashboard: defaultDashboardState() };
}
...
function normalizeState(obj) {
  ...
  // [SH-1 P1 / 로드맵 Phase 3·G] schemaVersion 이행 — 입력 버전 이후 도입된 위젯을 기본 숨김 union(멱등).
  //   ⚠️ undefined < N === false 함정 — 부재(undefined)를 명시적으로 최고참 레거시로 분기한다.
  //   이미 위젯을 직접 노출(un-hide)했다면 재숨김하지 않는다(이미 있으면 no-op).
  const inputVer = (typeof obj.schemaVersion === 'number' && Number.isFinite(obj.schemaVersion)) ? obj.schemaVersion : undefined;
  hiddenWidgets = unionNewHidden(hiddenWidgets, inputVer);
  ...
  const homeWidgetSizes = normalizeHomeWidgetSizes(obj.homeWidgetSizes); // [홈 위젯 크기] 폭·높이(직교 필드)
  const scratchpad = normalizeScratchpad(obj.scratchpad); // [로드맵 Phase 3·G] 스크래치패드 메모(전역 콘텐츠)
  // [로드맵 Phase 2] 대시보드(프리셋) — 레거시 키(homeLayout/hidden/sizes)가 '활성 뷰'의 권위(authoritative).
  //   normalizeState 는 활성 프리셋을 현재 레거시 키에 맞춰 reconcile 하고, 나머지 프리셋 내용은 dashboard 에
  //   보존한다. dashboard 부재(레거시 사용자)면 현재 키로 이행(shelf-union 반영된 hiddenWidgets 사용).
  //   프리셋 전환은 IPC 가 레거시 키를 대상 프리셋 내용으로 스왑 → 렌더 경로/데이터 하위호환.
  const dashboardNorm = normalizeDashboardState(isPlainObject(obj.dashboard)
    ? obj.dashboard
    : migrateLegacyToDashboard({ homeLayout, hiddenWidgets, homeWidgetSizes }));
  const dashboard = presetUpdate(dashboardNorm, dashboardNorm.activePreset, { layout: homeLayout, hidden: hiddenWidgets, sizes: homeWidgetSizes });
  return { schemaVersion: SCHEMA_VERSION, favorites, order, sortMode, names, theme, todos, langTrend, homeLayout, hiddenWidgets, homeWidgetSizes, briefing, aiUsage, shelfBookmarks, scratchpad, dashboard };
}
