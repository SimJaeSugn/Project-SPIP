...
/** [MD-1] 마크다운 편집기 문서 파일 절대경로 — <appDir>/markdown/md-docs.json
 *   ui-state.json(1MB 상한·매 UI 조작마다 재기록)과 분리한다 — 문서 본문은 UI 상태보다 크고
 *   저장 빈도·수명이 달라, 섞으면 UI 상태 상한을 잠식하고 한쪽 손상이 다른 쪽을 날린다. */
function mdDocsPath() {
  return path.join(appDir(), 'markdown', 'md-docs.json');
}
...
module.exports = { appDir, configPath, cachePath, uiStatePath, mailArchivePath, mdDocsPath, ensureDirFor, APP_NAME, DIR_MODE };
