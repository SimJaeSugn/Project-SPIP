...
/**
 * [탐색기 위젯] 실경로가 루트(그 자신 포함)의 하위인지 검사한다.
 *
 * isAllowed()는 **정확 일치**만 허용하는 강한 계약이라 "루트 아래를 걸어 다니는" 탐색기에는 쓸 수 없다.
 * 대신 폴드 키에 **구분자 경계**를 붙여 접두 매칭한다 — 'C:\dev' 가 'C:\dev-old' 를 삼키지 않는다.
 * 입력은 반드시 canonicalize() 를 통과한 실경로여야 한다(심링크/junction 이탈은 canonicalize가 해소).
 *
 * @param {string} canonicalPath canonicalize() 된 대상 실경로
 * @param {string} rootPath canonicalize() 된 루트 실경로
 * @returns {boolean}
 */
function isWithinRoot(canonicalPath, rootPath) {
  const p = foldForCompare(canonicalPath);
  const r = foldForCompare(rootPath);
  if (!p || !r) return false;
  if (p === r) return true; // 루트 자기 자신
  const sep = IS_WIN ? '\\' : '/';
  // POSIX 루트('/')는 foldForCompare가 후행 구분자를 남긴다 — 이미 경계가 있으면 덧붙이지 않는다.
  const prefix = r.endsWith(sep) ? r : r + sep;
  return p.startsWith(prefix);
}

/**
 * [탐색기 위젯] 실경로가 화이트리스트 루트 중 하나의 하위인지 찾는다.
 * @param {string} canonicalPath canonicalize() 된 대상 실경로
 * @param {string[]} roots canonicalize() 된 루트 실경로 배열
 * @returns {string|null} 포함하는 루트(원형) 또는 없으면 null
 */
function findContainingRoot(canonicalPath, roots) {
  const list = Array.isArray(roots) ? roots : [];
  for (const r of list) {
    if (typeof r === 'string' && isWithinRoot(canonicalPath, r)) return r;
  }
  return null;
}
