'use strict';
/**
 * lib/common/pathPolicy.js — 사용자 지정 임의 경로의 보안 정책 (SH-1, ADR-SH-4)
 *
 * 사용자가 직접 고른 경로(셸프의 folder/file 북마크, 탐색기 위젯의 열람 루트·항목)는 프로젝트가
 * 아니므로 scanRoots 화이트리스트 강제가 부적절하다. 대신 다음 2중 정책을 적용한다(설계 §10).
 *   ① pathGuard.canonicalize(H-1) — 심링크/junction 해소·NFC·절대경로화. 실패 시 PATH_GONE.
 *   ② 민감/시스템 경로 deny 게이트 — 드라이브 루트·OS 시스템 디렉토리·자격(비밀) 디렉토리 차단.
 *        통과(=거부) 시 PATH_DENIED.
 * gate(ref)는 등록·사용 양쪽에서 호출해 재게이트한다(TOCTOU 축소). 저장 후 경로가 시스템
 * 디렉토리로 바뀌었으면 사용 시점에 거부된다.
 *
 * 비교는 pathGuard.foldForCompare로 폴드(대소문자 비민감 FS 일관). 시스템/자격 디렉토리는
 * 접두(하위 포함) 매칭으로 차단하되, 사용자 홈의 일반 하위(프로젝트·문서 등)는 허용한다.
 *
 * [이동 이력] 원래 lib/shelf/pathPolicy.js. 탐색기 위젯(browsePolicy)이 같은 deny 게이트를
 *   재사용해야 해 도메인 교차 참조를 피하려고 lib/common/으로 승격했다(동작 불변).
 *   lib/shelf/pathPolicy.js 는 하위호환 재수출 shim.
 *
