...
const OUTPUT_CONTRACT = [
  ...
  '특수 항목 처리: type "deadline"은 할 일 마감이다 — context의 "다가오는 마감"/"마감 지남(미완료)"을 구분해',
  '  남은 시간·우선순위·처리 순서를 조언한다. type "usage"는 토큰 사용량 리포트다 — context의 수치(연결 모델·Claude Code,',
  '  최근7일 추세)를 바탕으로 사용량 추이를 요약하고 비용 절감·프롬프트 최적화 같은 실질적 아이디어를 제시한다.',
...
function clampContext(signalType, context) {
  ...
  if (signalType === 'usage') return clamp(context, C.COMMIT_MSG_MAX); // 토큰 사용량 요약(추이 수치) — 여유분
