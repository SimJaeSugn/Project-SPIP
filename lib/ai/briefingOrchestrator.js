...
class BriefingOrchestrator {
  ...
  constructor(deps) {
    ...
    // [브리핑 토큰리포트] 토큰 사용량 요약 문자열 제공자(main/context 주입). () => string|null. 미주입 시 usage 항목 생략.
    this._usageProvider = typeof deps.usageProvider === 'function' ? deps.usageProvider : null;
  ...
  async _fire() {
    ...
    // [브리핑 토큰리포트] 합성 'usage' 항목 — 매 생성마다 최신 사용량으로 갱신(비영속). provider 미주입 시 생략.
    //   신호/diff가 아니라 항상 현재 수치를 리포트하므로 carry-over·persist를 타지 않고 이번 생성에만 싣는다.
    const usageItem = this._buildUsageItem(now);
    const promptItems = usageItem ? sel.items.concat([usageItem]) : sel.items;
    ...
    const { system, user } = prompt.buildPrompt({ items: promptItems, carryOver: [] }, { systemPrompt });

    let r;
    try {
      // 스트리밍 가독성: 누적 버퍼를 매 토큰 관대 파싱해, 날 JSON 대신 **완성된 항목만** delta에 실어 보낸다.
      //   JSON 모드면 렌더러가 부분 항목 카드를 점진 표시(원문 숨김), 평문 모드면 원문을 그대로 표시.
      let streamBuf = '';
      let lastItemCount = -1;
      r = await this._llm.streamBriefing({
        ...
        onDelta: (token) => {
          ...
          streamBuf += token;
          const partial = parse.parseStreaming(streamBuf);
          const payload = { gen, chunk: token, mode: partial.mode };
          // 완성 항목 수가 바뀐 경우에만 항목 배열 동봉(IPC 페이로드 절약 — 닫힌 객체는 더 안 변함).
          if (partial.items.length !== lastItemCount) {
            payload.items = partial.items;
            lastItemCount = partial.items.length;
          }
          this._pushDelta(payload);
        },
      });
    } catch (err) {
    ...
    const finalItems = this._mapViews(promptItems, parsed);

    // 영속 병합(carry-over 기준 갱신) — 상승 세션이면 saveItems가 메모리 폴백.
    //   [code-review #1] lastSnapshot 영속(재시작 과트리거 방지). [code-review #2] generated++(N-10).
    //   [브리핑 토큰리포트] usage 항목은 비영속 — persist 기준에서 제외(표시에는 포함).
    const persistBase = usageItem ? finalItems.filter((it) => it.key !== usageItem.key) : finalItems;
    const persistItems = items.mergePersist(persisted.items, persistBase, sel.resolvedKeys, { now });
  ...
  /**
   * [브리핑 토큰리포트] 합성 'usage' 항목 생성 — provider가 준 요약 문자열을 context로 싣는다.
   *   AI가 이 항목의 title/reason/guide를 토큰 사용량 추이 리포트로 채운다. provider 부재/빈 값이면 null.
   *   비영속(매 생성 갱신)이며 key는 고정('usage' 안정 키)이라 같은 카드가 갱신된다.
   */
  _buildUsageItem(now) {
    if (!this._usageProvider) return null;
    let summary = null;
    try { summary = this._usageProvider(); } catch (_) { summary = null; }
    if (typeof summary !== 'string' || !summary.trim()) return null;
    return {
      key: items.itemKey('usage', 'usage'),
      signalType: 'usage',
      targetId: 'usage',
      targetLabel: '토큰 사용량',
      category: 'good',
