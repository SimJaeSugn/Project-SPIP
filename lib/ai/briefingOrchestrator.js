...
class BriefingOrchestrator {
  ...
  async _fire() {
    ...
    const { system, user } = prompt.buildPrompt({ items: sel.items, carryOver: [] });
    ...
    this._saveItems({ items: persistItems, lastSnapshot: snap, counterDelta: { generated: 1 } });
  ...
  _mapViews(signalItems, parsed) {
    ...
    for (const it of signalItems) {
      ...
      out.push(Object.assign({}, it, {
        title: items.sanitizeText((v && v.title) || it.title || it.targetId || it.signalType, C.PARSE_TITLE_MAX),
