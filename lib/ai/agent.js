...
async function planStep(llm, args) {
  ...
  user += '\n하위 작업 계획을 JSON 하나로만 출력하라: {"plan":["1단계 ...","2단계 ..."]} (요청 수행에 실제로 필요한 최소 단계만, 도구로 수행 가능한 단위로).' +
    '\n중요: 억지로 단계를 늘리지 마라. 도구 한 번이면 끝나는 단순·원자적 요청(예: "…을 할 일에 추가")은 1단계로 계획하라.' +
    ' 요청에 없는 작업(문서 만들기 등)을 임의로 덧붙이거나, 하나의 항목을 여러 단계로 쪼개지 마라.';
  let r;
