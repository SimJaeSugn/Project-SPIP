 ...
 *   {
 *     projects: [ { id, dirty:bool, ahead:int, behind:int, attention:bool } ],
 *     deadlines: [ { id, dueAt:number|null, done:bool } ],   // 할 일 마감
...
function evaluate(prevSnap, curSnap, opts) {
  ...
  for (const p of cur.projects) {
    ...
    if (p.dirty && (!was || !was.dirty)) {
      newDirtyCount += 1;
      signals.push(mk('dirty', p.id, CATEGORY.MUST, false));
    }
    // 미푸시(ahead): 0→양수 또는 증가
    if (p.ahead > 0 && (!was || p.ahead > was.ahead)) {
      signals.push(mk('ahead', p.id, CATEGORY.MUST, false));
    }
    // 받을 커밋(behind): 0→양수 (급함·fast-path)
    if (p.behind > 0 && (!was || was.behind === 0)) {
      signals.push(mk('behind', p.id, CATEGORY.URGENT, true));
    }
    // 주의 필요 집합 새 진입(상태형)
    if (p.attention && (!was || !was.attention)) {
      signals.push(mk('attention', p.id, CATEGORY.GOOD, false));
    }
  }
  ...
  for (const d of cur.deadlines) {
    ...
    if (!wasWithin) signals.push(mk('deadline', d.id, CATEGORY.URGENT, true));
  }
...
function mk(type, targetId, category, urgent) {
  return { type, targetId: String(targetId), category, urgent: urgent === true, meta: {} };
}
