...
function createLlmClient(deps) {
   ...
   *   opts.temperature·opts.maxTokens 를 주면 그 호출에 한해 config 값을 덮어쓴다(예: 마크다운 문법
   *   교정은 결정적 출력이 필요해 temperature 0 로 호출). 미지정이면 기존 config 동작 그대로. */
  function buildModel(opts) {
    ...
    return chatFactory({
      ...
      temperature: (typeof opts.temperature === 'number') ? opts.temperature
        : (typeof b.temperature === 'number') ? b.temperature : C.DEFAULT_TEMPERATURE,
      maxTokens: (typeof opts.maxTokens === 'number') ? opts.maxTokens
        : (typeof b.maxTokens === 'number') ? b.maxTokens : C.DEFAULT_MAX_TOKENS,
      streaming: true,
      streamUsage: true, // [항목3] 스트림 마지막 청크에 usage_metadata 포함(토큰 사용량 집계용).
  ...
  /**
   * [항목3] chunk에서 토큰 usage 추출(Langchain AIMessageChunk.usage_metadata).
   *   { input_tokens, output_tokens, total_tokens } → 표준 키로 정규화. 없으면 null.
   *   스트림 마지막 청크에만 실리므로, 호출처는 가장 최근 non-null 값을 채택한다.
   */
  function usageFromChunk(chunk) {
    const um = chunk && (chunk.usage_metadata || (chunk.response_metadata && chunk.response_metadata.usage));
    if (!um || typeof um !== 'object') return null;
    const num = (v) => (typeof v === 'number' && Number.isFinite(v) && v >= 0) ? Math.floor(v) : 0;
    const prompt = num(um.input_tokens != null ? um.input_tokens : um.prompt_tokens);
    const completion = num(um.output_tokens != null ? um.output_tokens : um.completion_tokens);
    const total = num(um.total_tokens != null ? um.total_tokens : (prompt + completion));
  ...
  async function streamBriefing(args) {
    ...
    let text = '';
    let capped = false;
    let usage = null; // [항목3] 가장 최근 non-null usage_metadata(보통 마지막 청크).
    try {
      // 호출별 temperature·maxTokens 오버라이드 지원(미지정이면 config 기본).
      const model = buildModel({ temperature: args.temperature, maxTokens: args.maxTokens });
      const stream = await model.stream(messages, { signal: inner.signal });
      for await (const chunk of stream) {
        // [항목3] usage는 텍스트 없는 마지막 청크에 실릴 수 있어 텍스트 분기보다 먼저 채집한다.
      ...
      return { ok: true, text, code: CODE.OK, usage };
    } catch (err) {
      if (args.signal && typeof args.signal.removeEventListener === 'function') args.signal.removeEventListener('abort', onExternalAbort);
      // 상한 도달로 우리가 abort한 경우 — 부분 결과를 정상 종료로 취급(서버측 무한 스트림 방어).
      if (capped) return { ok: true, text, code: CODE.OK, usage };
      ...
      return { ok: false, text, code, usage };
    }
  }
