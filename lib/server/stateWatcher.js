...
function normalizeGit(git) {
  ...
  return {
    ...
    changes: ok ? (typeof git.changes === 'number' ? git.changes : null) : null,
    remoteHost: ok ? (typeof git.remoteHost === 'string' ? git.remoteHost : null) : null,
    remoteSlug: ok ? (typeof git.remoteSlug === 'string' ? git.remoteSlug : null) : null,
    remoteWeb: ok ? (typeof git.remoteWeb === 'string' ? git.remoteWeb : null) : null,
    remoteUrl: ok ? (typeof git.remoteUrl === 'string' ? git.remoteUrl : null) : null,
  };
}
...
function stateChanged(prev, next) {
  ...
  if (pg.status !== ng.status || pg.isRepo !== ng.isRepo || pg.branch !== ng.branch
    || pg.dirty !== ng.dirty || pg.ahead !== ng.ahead || pg.behind !== ng.behind || pg.changes !== ng.changes
    || pg.remoteHost !== ng.remoteHost || pg.remoteSlug !== ng.remoteSlug || pg.remoteWeb !== ng.remoteWeb) return true;
