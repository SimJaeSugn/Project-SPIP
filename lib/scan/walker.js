...
function* walkRoot(rootCanonical, ctx) {
  ...
  while (stack.length > 0) {
    ...
    for (const ent of entries) {
      ...
      // 사용자 절대경로 제외(#4): canonical 실경로가 제외 경로(또는 하위)면 진입 안 함.
      if (excludeRules.isUnderExcludedPath(childCanonical, ctx.excludeSet)) {
        continue;
      }
      // 사용자 정규식 제외(#4): 전체 경로가 정규식에 매칭되면 진입 안 함(앞뒤 임의 경로 + 가운데 패턴).
      if (excludeRules.matchesExcludeRegex(childCanonical, ctx.excludeSet)) {
        continue;
      }
...
function* walk(roots, opts) {
  ...
  for (const root of list) {
    ...
    // 사용자 절대경로/정규식 제외(#4)가 루트 자체를 가리키면 통째로 건너뛴다.
    if (excludeRules.isUnderExcludedPath(rootCanonical, excludeSet) || excludeRules.matchesExcludeRegex(rootCanonical, excludeSet)) {
      logger.warn('제외 경로 루트를 건너뜁니다', { path: rootCanonical });
      continue;
    }
