...
function buildProject(projectPath, signals, collected) {
  ...
  return {
    ...
    git: {
      ...
      changes: git.status === 'ok' ? (typeof git.changes === 'number' ? git.changes : null) : null,
      remoteHost: git.status === 'ok' ? (typeof git.remoteHost === 'string' ? git.remoteHost : null) : null,
      remoteSlug: git.status === 'ok' ? (typeof git.remoteSlug === 'string' ? git.remoteSlug : null) : null,
      remoteWeb: git.status === 'ok' ? (typeof git.remoteWeb === 'string' ? git.remoteWeb : null) : null,
      remoteUrl: git.status === 'ok' ? (typeof git.remoteUrl === 'string' ? git.remoteUrl : null) : null,
    },
    size: {
