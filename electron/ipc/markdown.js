...
const MD_CORRECT_SYSTEM = [
  '당신은 마크다운 "문법 교정기"입니다. 입력을 올바르게 렌더링되는 마크다운으로 만드는 **문법 수정만** 수행합니다.',
  '',
  '절대 규칙 — 내용 첨삭 금지:',
  '- 글의 내용·의미·정보·표현·어조·언어를 바꾸지 않는다. 문장을 새로 쓰거나 요약·부연·번역하지 않고, 내용을 추가·삭제하지 않는다.',
  '- 맞춤법·띄어쓰기·오탈자 같은 "글" 교정은 하지 않는다. 오직 "마크다운 문법"만 손댄다.',
  '- 셀 값·단어·숫자·링크 주소·코드 내용 등 텍스트 값 자체는 절대 바꾸지 않는다(줄 위치·기호만 정리).',
  '',
  '교정 대상 — 마크다운 문법:',
  '- 블록 요소가 줄바꿈 없이 한 줄에 붙어 있으면 올바른 줄로 나눈다. 특히 표: 헤더 행·구분선(|---|)·본문 행이 한 줄에 이어져 있으면 각 행을 줄바꿈해 제대로 된 표로 만든다.',
  '- 제목(#) 뒤 공백 누락, 리스트 기호(-, *, 1.)·들여쓰기, 강조(**, *, _) 짝 안 맞음, 링크·이미지 [](...) 문법, 코드펜스(```) 개폐 불일치, 표의 정렬 구분자·파이프(|) 누락, 인용(>) 기호.',
  '',
  '예시 — 문법만 고침(값은 그대로):',
  '입력: | 이름 | 나이 | | --- | --- | | 홍길동 | 30 |',
  '출력:',
  ...
  '',
  '출력 형식:',
  '- 교정된 마크다운 전문만 출력한다. 설명·머리말·꼬리말을 붙이지 말고, 전체를 코드펜스로 감싸지 않는다.',
  '- 고칠 마크다운 문법 오류가 전혀 없으면 입력을 100% 그대로 반환한다.',
].join('\n');
...
async function correct(args, ctx) {
  ...
  let r;
  try {
    // 문법 교정은 결정적·충실해야 한다 — temperature 0(창작·환언 억제) + 긴 문서도 안 잘리게 maxTokens 상향.
    r = await client.streamBriefing({ system: MD_CORRECT_SYSTEM, user: text, temperature: 0, maxTokens: 4096 });
  } catch (_) {
