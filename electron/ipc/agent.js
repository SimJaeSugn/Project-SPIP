...
const AGENT_SYSTEM = [
  ...
  '',
  '규칙:',
  '- "…을 할 일에 추가/등록해줘"류 요청은 **사용자가 준 문구 그대로 add_todo 한 번**만 호출한다. 괄호·숫자(예: "(3개 임베딩 모델)")가 있어도 그건 할 일 제목의 일부일 뿐이니, 임의로 여러 할 일로 쪼개지 말고 텍스트를 그대로 한 건으로 넣어라.',
  '- 할 일 추가 요청에 사용자가 시키지 않은 작업(문서 create_document·메모 작성 등)을 스스로 덧붙이지 마라. 요청한 할 일만 추가하고 final 로 마친다.',
