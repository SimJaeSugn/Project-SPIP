...
| Phase 4 — 팔레트(D) | 🔶 육안 대기 | **액션 레지스트리 구현**(`buildActions` + 순수 `filterActions`/`actionMatchScore`) + **Cmd+K 커맨드 팔레트**(뷰전환·설정·위젯추가·프리셋전환·내보내기·테마 등, 방향키·부분교체). H(딥링크)·I(포커스)는 후속 |
| Phase 4 — 딥링크(H)·포커스(I) | ⬜ 미착수 | 액션 레지스트리 소비 계층(팔레트와 동일 레지스트리) |
