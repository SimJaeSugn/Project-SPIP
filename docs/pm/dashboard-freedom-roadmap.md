...
| Phase 5 — 스택(F) | 🔶 육안 대기 | 위젯 스택 — 그룹 `mode='stack'` 재사용. 한 자리에 겹친 멤버 중 `active` 하나만 표시 + 하단 인디케이터(점·‹›)로 로테이션. 위젯처럼 리사이즈·드래그(masonry 셀/freeform 자유). 섹션↔스택 전환(⧉). `onStackSetActive` 부분 교체 |

**배포**: v1.25.0 (Phase 5-M 그룹/섹션 + 그룹 내부 masonry·프리폼 그룹·좌우여백/치우침 보정). 테스트 **1238 그린**. 커밋 `5c096f5`→`908c528`(master).
직전: v1.24.0 (Phase 5-B 프리폼, `bb490ca`→`2c1bbb4`). v1.23.0 (Phase 4 팔레트·포커스·딥링크). v1.22.0 (Phase 1-K + Phase 3). v1.21.0 (Phase 0~2).
