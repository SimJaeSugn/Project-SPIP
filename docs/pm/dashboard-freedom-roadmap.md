...
| Phase 1 — 템플릿 갤러리(L) | 🔶 육안 대기 | 레이아웃 템플릿(미니멀·집중·개발현황·메모·올인원) 갤러리 → 고르면 **새 프리셋**으로 적용. `HOME_TEMPLATES`·`buildTemplatePreset`(순수)·IPC `addTemplatePreset`(프리셋 정규화 단일 경계). 편집 모드 `템플릿` 버튼 |
...
| Phase 5 — 그룹(M) | ✅ **배포(v1.25.0)** | 그룹/섹션 — 추가/이름변경/삭제/멤버 배정·제거. masonry: 접기 밴드 + 그룹 내부 masonry(멤버 리사이즈·순서변경) + 그룹 순서 드래그. freeform: 그룹 블록도 자유 배치. featureAdd 최하단·좌측치우침 보정·프리폼 좌우여백 대칭 포함. 육안 확인 완료 |
| Phase 5 — 스택(F) | ✅ **배포(v1.26.0)** | 위젯 스택 — 그룹 `mode='stack'` 재사용. 한 자리에 겹친 멤버 중 `active` 하나만 표시 + 하단 인디케이터(점·‹›)로 로테이션. 위젯처럼 리사이즈·드래그. 섹션↔스택 전환(⧉). 육안 확인 완료 |

**배포**: v1.26.0 (Phase 5-F 스택 — 로드맵 Phase 0~5 전 기능 완결). 테스트 **1240 그린**. 커밋 `99b6c90`(master).
직전: v1.25.0 (Phase 5-M 그룹). v1.24.0 (Phase 5-B 프리폼). v1.23.0 (Phase 4 팔레트·포커스·딥링크). v1.22.0 (Phase 1-K + Phase 3). v1.21.0 (Phase 0~2).

> 🎉 **로드맵 완주**: Phase 0~5 전 기능 배포 완료(A~N 중 J 테마·L 템플릿만 후속 보류). 남은 것 = Phase 1-J(색상 토큰 리팩터 선행), 1-L(템플릿 갤러리).
