...
| Phase 5 — 그룹(M) | 🔶 육안 대기 | 그룹/섹션(`{id,name,collapsed,members}`) — 추가/이름변경/삭제/멤버 배정·제거. **masonry**: 전체폭 접기 밴드 + 그룹 내부 masonry(멤버 리사이즈·순서변경) + 그룹 순서 드래그. **freeform**: 그룹 블록도 **자유 배치 셀**(좌표·폭 화이트리스트에 그룹 id 허용, 드래그 이동). `layoutMasonryGrid` 메인/그룹 공용·`normalizeGroups` 단일 신뢰 경계 |
