...
| Phase 1 — 내보내기/가져오기(K) | ✅ 완료 | 백엔드(serialize/IPC) + 렌더러 버튼. **육안 확인 완료** |
...
| Phase 3 — 밀도(C) | 🔶 육안 대기 | `densityTier(px)→S\|M\|L` + `data-density` 훅 + **메일 위젯 showcase 소비**(S=숫자요약/M=목록·시간숨김/L=목록+시간). 타 위젯은 훅 재사용으로 점진 채택 |
| Phase 3 — 신규 위젯(G) | 🔶 진행중 | ①스크래치패드 ✅ · ②통합 커밋 히트맵 ✅(육안대기) → ③시스템 상태. 히트맵: `getCommitActivity(days=365)` 재사용·`buildHeatmapModel` 순수·기본숨김(opt-in)·표시 시 지연로드. 둘 다 schemaVersion 3 이행 |
