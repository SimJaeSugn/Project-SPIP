...
| Phase 1 — 테마 액센트/밀도(J) | ✅ **배포(v1.28.0)** | 액센트 토큰화 + **6색 프리셋**(data-accent 라이트/다크) + **화면 배율**(compact~large, zoom). 설정 스와치·배율. IPC `setThemePrefs`. 육안 확인 완료 |
...
**배포**: v1.28.0 (Phase 1-J 테마 개인화 — **로드맵 A~N 전 기능 완결**). 테스트 **1246 그린**. 커밋 `b854b5e`(master).
직전: v1.27.0 (L 템플릿). v1.26.0 (F 스택). v1.25.0 (M 그룹). v1.24.0 (B 프리폼). v1.23.0 (Phase 4). v1.22.0 (K + Phase 3). v1.21.0 (Phase 0~2).

> 🎉🎉 **로드맵 완주**: A~N 14개 기능 전부 구현·배포 완료(A 프리셋·B 프리폼·C 밀도·D 팔레트·E 위젯설정·F 스택·G 신규위젯·H 딥링크·I 포커스·J 테마·K 내보내기·L 템플릿·M 그룹·N 편집모드).
