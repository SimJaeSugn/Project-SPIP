...
| Phase 4 — 팔레트(D) | 🔶 육안 대기 | **액션 레지스트리**(`buildActions` + 순수 `filterActions`/`actionMatchScore`) + **Cmd+K 팔레트**(뷰전환·설정·위젯·프리셋·내보내기·테마·포커스·프로젝트점프, 방향키·부분교체) |
| Phase 4 — 포커스(I)·딥링크(H) | 🔶 육안 대기 | I: 위젯 포커스 버튼 + 풀스크린 오버레이(종료 시 masonry 재측정·컨테이너 컨텍스트 재사용). H: 팔레트 **프로젝트 점프**(→상세 드로어). 데이터 드래그(위젯간 DnD)는 후속(SortableJS·리사이즈 충돌 회피 설계 필요) |
| Phase 5 — 프리폼(B)·그룹(M)·스택(F) | ⬜ 미착수 | 최고 난이도, 검증 필수 |

**배포**: v1.22.0 (Phase 1-K 렌더러 + Phase 3 밀도·신규 위젯 3종). 테스트 **1223 그린**. 커밋 `1a198af`→`63d5ee4`(master). 전 증분 하위호환·신규 위젯 기본숨김.
직전: v1.21.0 (Phase 0~2 + 편집 모드 + K 백엔드).
