...
- **카드 순서 변경 · 즐겨찾기** — 카드를 **드래그(또는 키보드)로 순서 변경**하면 수동 순서로 전환되어 다음 실행에도 유지되고, 정렬을 다시 고르면 자동 정렬로 돌아갑니다. 카드를 **즐겨찾기**에 등록하고 '즐겨찾기만' 필터로 추릴 수 있습니다(순서·즐겨찾기는 앱 데이터 폴더의 `ui-state.json`에 보관).
- **트레이 상주** — 창을 닫아도 종료되지 않고 **트레이에 남습니다**(백그라운드 스캔 유지). 트레이 우클릭 메뉴로 **대시보드 열기 / 즐겨찾기 / 종료**를 제공하고, '즐겨찾기'를 누르면 등록한 카드를 좌우로 넘겨 보는 **슬라이더 오버레이**가 뜹니다. 완전히 끄려면 트레이 메뉴의 **종료**를 사용합니다.
...
| `test` | `node --test "test/**/*.test.js"` | 단위·통합 테스트 (416건) |
...
└─ test/                 # node:test 기반 단위·통합 테스트 (416건)
```
